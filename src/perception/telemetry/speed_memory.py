...
class SpeedTelemetryReader:
    ...
    def read_telemetry_if_due(self, now_ts: float | None = None):
        """Return (speed_kmh, dist_turn_m) tuple; each value is None if unavailable."""
        ...
        try:
            if self._pm is None or self._speed_address is None or self._dist_turn_address is None:
                self._connect()
